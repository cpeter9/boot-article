...
\title{\vspace{-15mm}\fontsize{24pt}{10pt}\selectfont\textbf{Using bootstrapping and Monte Carlo simulation to assess Type 1 Error rates and Test Power}} % Article title
...
\noindent Much of what young statisticians in-training learn in undergraduate and graduate school relies on large-sample asymptotic theory.  Well known statistics are calculated and with a few assumptions a standard error is developed to measure inference reliability.  Unfortunately data are not always sufficiently large such that finite sample properties match those required by large-sample asymptotics.  In addition, a careful examination of assumptions is always required for successful inference.  In some cases common checks of assumptions may not be possible with a high degree of power, especially in cases of  small samples.  This analysis explores type 1 error rates and test power in two cases.  The first case is a comparison between the two-sample t-test and the Wilcoxon-Mann-Whitney (Mann U) test.  The second case compares four popular tests for departure from normality.  These tests include the Shapiro-Wilk tests, Kolmogorov-Smirnov test, Anderson-Darling test, and Jarque-Bera test.  Type 1 error is assessed by calculating rejection rates from Monte Carlo simulated normal distributions.  Power is assessed by calculating rejection rates corresponding to the uniform, gamma, and $\chi^2$ distributions.
...
\section{A comparison of the two-sample t-test and Wilcoxon-Mann-Witney test}

Power calculations are generally considered good pre-requisite for any designed experiment.  Such calculations allow researchers to determine reasonable sample size requirements before carrying out an experiment.  These calculations can help researchers avoid carrying out experiments doomed in the sense of statistical detection before experimentation begins.

The following example takes a look at how power calculations can be carried out and verified by Monte Carlo simulation and bootstrapping sample data.  In this case a web app company wants to perform an experiment on a business practice.  The company has found a relationship between earning tokens on the site and customer longevity and financial value.  The company decides that it would like to encourage its customers to earn more tokens by sending an email.  However, the company would also like to compare the effect of the email against a control group of customers that did not receive the email.  A two-sample t-test is selected to test for a difference between the mean number of tokens earned in the experiment group and control group.
...
\newpage
The WMW statistic is calculated as follows:
...
\item Administer respective test, calculate p-value and store
...
\section{Using Monte Carlo simulations and bootstrapping to test for normality in small samples}

Normality is a common assumption across many statistical procedures such as Analysis of Variance (ANOVA), linear regression, t-tests and z-tests.  When normality assumptions are violated, statistics that make up the aforementioned methods are not valid and may leader to incorrect inference.  There are two main ways that normality is commonly assessed: graphical methods, including histograms plotted against densities and Q-Q plots.  The second method involves the use of one of a number of statistical tests for normality.

Small sample sizes typically pose problems for statistical tests of normality.  The major challenge is that such tests have lower power in small samples and also that small samples may exhibit properties that fool tests for normality.  Type 1 error rates, that is rejecting the null hypothesis that the sample data were collected from a population with a normal distribution, may become unstable in small sample sizes.
...
The Kolmogorov-Smirnov procedure is as follow:
...
Unfortunately, many practitioners neglect the significance of normality assumed as the null hypothesis.  Tests of normality actually test for departures from normality.  Therefore normality is never ``proved'' or accepted as the null hypothesis is never accepted.  Contrary, the alternative hypothesis, only a departure from normality can be accepted statistically.  This confusion leads many young practitioners to apply their favorite test, note that the probability value is less than 0.05 and continue on with Analysis of Variance or linear regression, etc.  Not only do many (especially young) practitioners reverse the hypotheses, but if they do setup the hypotheses correctly, the second error of claiming to have ``proved'' normality is common.
...
Figure~\ref{type1_monte_carlo} shows Type 1 error rates by test by sample size.  The Shapiro-Wilk, Kolmogorov-Smirnov, and Anderson-Darling tests all appear to have Type 1 error rates that are well controlled around 5\% (as specified).  The Jarque-Bera test's Type 1 error rates are surprisingly lower than 5\% in samples smaller than 50 observations.  The JB Type 1 error rates increase from 10 through about 75 observations at which point they flatten out around 4\%.  This is perhaps not as surprising as it may seem as the Jarque-Bera is a relatively simple test, seemingly constructed more for ease of use rather than precise error control.

Statistical power is a second important concern when it comes to administering tests for departures from normality.  Power, also called the inverse of Type II error rate is the ability of a statistical test to reject the null hypothesis in the presence of a state that in which the null hypothesis should be rejected.  Statistical power is also referred as the ability of a test to detect that which is represented by the alternative hypothesis.  Figure~\ref{power_monte_carlo} shows statistical power for each statistical test by sample size.

Statistical tests for departure from normality were assessed over three distributions: chi-square, gamma, and uniform.  The Shapiro-Wilk test appears to have the best power among the three tests for use on the chi-square and gamma distribution.  The Kolmogorov-Smirnov test has nearly perfect power against the uniform distribution from sample size 10.  The Jarque-Bera test presents results that are expected against the chi-square and gamma distributions, but has a strange binomial swing in power as applied to the uniform distribution.  In all three tests, the Shapiro-Wilk test appears to dominate the Anderson-Darling and Jarque-Bera tests.

The Shapiro-Wilk test appears to be the majority winner among the three distributions applied to test.  The test reaches nearly perfect power against the chi-square around 50 observations and around 75 against the gamma distribution.

Interestingly, the Kolmogorov-Smirnov test appears to have very low power against the chi-square and gamma distributions as specified, but nearly perfect power against the uniform distribution.  The KS test is noted to have relatively low power in the literature~\citep{razali2011power}.

The Jarque-Bera test ranks third out of the four other tests for power applied to the $\chi^2$ and gamma generated data.  The test  converges to perfect power rather slowly in the instance of uniformly distributed random data.  By 100 observations, the test has achieved only about 60\% power, while all other tests have achieved nearly perfect power.
...
\begin{figure}[h!]
<<fig_1, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='6in', out.height='5in', cache=TRUE>>=
library(ggplot2)
library(reshape2)
library(tikzDevice)
library(ggthemes)
...
ggplot(raw.samples, aes(x = value)) +
  ...
  xlab("Tokens Earned") +
  ylab("Density") +
  scale_x_continuous(breaks = seq(0, 50, 2), limit = c(0, 30)) +
  theme_few() +
  theme(strip.text = element_text(size = 24),
        axis.text = element_text(size = 14),
        axis.title = element_text(size = 20)) +
  stat_function(fun = dnorm,
              args = c(mean = mean(raw.samples$value),
                       sd = sd(raw.samples$value)),
              colour = "blue", size = 1)
@
\caption{Histogram of token earning in two random samples.  Blue lines indicate the best fit normal distribution.}
\label{fig:prob_1_raw_data}
\end{figure}

\begin{figure}[h!]
<<type1error, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='6in', out.height='5in', warning=FALSE, cache=TRUE>>=
library(ggplot2)
library(reshape2)
library(tikzDevice)
...
ggplot(output, aes(x = sample.size, y = value, colour = variable)) +
  ...
  theme_few() +
  scale_x_continuous(limit = c(0, 910)) +
  scale_colour_discrete(name = "Test Applied") + 
  theme(strip.text = element_text(size = 24),
        axis.text = element_text(size = 14),
        axis.title = element_text(size = 20),
...
@
\caption{Type 1 Error rate by Sample Size for three tests for departures from normality: t-test equal variances, t-test unequal variances, WMW distribution-free test.}
\label{type1error}
\end{figure}


\begin{figure}[h!]
<<power, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='5.5in', out.height='5in', warning=FALSE, cache=TRUE>>=
library(ggplot2)
library(reshape2)
library(tikzDevice)
library(ggthemes)
...
ggplot(output, aes(x = sample.size, y = value, colour = variable)) +
  ...
  theme_few() +
  scale_x_continuous(limit = c(0, 910)) +
  scale_colour_discrete(name = "Test Applied", labels = c("t-test eq. variances", "t-test uneq. variances", "WMW Test")) + 
  theme(strip.text = element_text(size = 24),
        axis.text = element_text(size = 14),
        axis.title = element_text(size = 20),
        legend.title = element_text(size = 20),
        legend.text = element_text(size = 16))
@
\caption{Above is a power comparison of three tests: t-test equal variances, t-test unequal variances, and the WMW test as applied to detect differences in two randomly-generated normally distributed variables with 10\% difference.}
\label{prob_1_power}
\end{figure}

\begin{figure}[h!]
<<raw_data_type_1, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='5.5in', out.height='5in', warning=FALSE, cache=TRUE>>=
library(ggplot2)
library(reshape2)
library(tikzDevice)
library(ggthemes)
...
ggplot(output, aes(x = sample.size, y = value, colour = variable)) +
  ...
  theme(strip.text = element_text(size = 24),
        ...
        legend.text = element_text(size = 16))
@
\caption{Type 1 error rates applied to actual Token data.  Each point represents 200 bootstrap samples.}
\label{raw_data_type_1}
\end{figure}

\begin{figure}[h!]
<<raw_data_power, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='5.5in', out.height='4.75in', warning=FALSE, cache=TRUE>>=
library(ggplot2)
library(reshape2)
library(tikzDevice)
library(ggthemes)
...
ggplot(output, aes(x = sample.size, y = value, colour = variable)) +
  ...
  theme(strip.text = element_text(size = 24),
        ...
        legend.text = element_text(size = 16))
@
\caption{Test power applied to actual Token data with one sample left as-is and the other scaled by 1.1.}
\label{raw_data_power}
\end{figure}


\begin{figure}[h!]
<<dists, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='5.5in', out.height='5in', warning=FALSE, cache=TRUE>>=
library(ggplot2)
library(reshape2)
library(tikzDevice)
library(ggthemes)
...
@
\caption{Distributions for which Type 1 error rates and test power are analyzed.}
\label{dists}
\end{figure}


\begin{figure}[h!]
<<type1, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='5.5in', out.height='5in', warning=FALSE, cache=TRUE>>=
...
ggplot(df[grep("norm", df$dist), ], aes(x = sample.size, y = power_type1, colour = factor(dist))) +
  geom_smooth() +
  geom_hline(yintercept = 0.05, size = 1, colour = "red") +
  facet_wrap(~ test) +
  theme_few() +
  ylab("Type 1 Error") +
  xlab("Sample Size") +
  scale_colour_discrete(name = "Distributions", label = c("Standard Normal", "Normal with variance 9", "Normal with variance 0.25")) +
  theme(strip.text = element_text(size = 24),
        axis.text = element_text(size = 14),
        axis.title = element_text(size = 20),
        legend.title = element_text(size = 20),
        legend.text = element_text(size = 16))
@
\caption{Type 1 error rates as calculated by Monte Carlo simulation with 1,000 replications at each sample size.}
\label{type1_monte_carlo}
\end{figure}

\begin{figure}[h!]
<<power_test, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='6in', out.height='4.5in', warning=FALSE, cache=TRUE>>=
# Loop over small sample sizes
library(ggplot2)
library(reshape2)
library(tikzDevice)
...
ggplot(df[-grep("norm", df$dist), ], aes(x = sample.size, y = power_type1, colour = factor(test))) +
  geom_smooth() +
  geom_point() +
  geom_hline(yintercept = 1, colour = "red", size = 0.5) +
  scale_y_continuous(limits = c(0, 1)) +
  facet_wrap(~ dist) +
  ylab("Test Power") +
  xlab("Sample Size") +
  scale_colour_discrete(name = "Test", labels = c("AD", "JB", "KS", "SW")) + 
  theme_few() +
  theme(strip.text = element_text(size = 20),
        axis.text = element_text(size = 14),
        axis.title = element_text(size = 20),
        ...
        legend.text = element_text(size = 16))
@
\caption{Test power by non-normal distribution.  Each test was applied 1,000 times at each sample size for each test of departure from normality.}
