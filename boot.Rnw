...
In order to assess type 1 error rates:
\begin{enumerate}
\item Take two random samples of the token data with replacement with for sample sizes $1\ldots 200$ by 10.
...
To prove the method, Figure~\ref{prob_1_rnorm} shows type 1 error rate for the two-sample t-test and WMW test in sample sizes ranging from 10 to 1000 with 500 bootstrap samples and tests performed at each sample size.  Figure\\

% There are these cases:
% 1. normal unequal variances {power, type 1}: t.test, t.test unequal, wmw
% 2. actual data {power, type 1}: t.test, t.test unequal, wmw
%
%
%
%
...
\begin{figure}[h!]
<<type1error, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='5in', warning=FALSE, cache=TRUE>>=
library(ggplot2)
library(reshape2)
library(tikzDevice)
...
@
\caption{test caption}
\label{prob_1_rnorm_type1}
\end{figure}


\begin{figure}[h!]
<<power, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='5in', warning=FALSE, cache=TRUE>>=
library(ggplot2)
library(reshape2)
library(tikzDevice)
...
\newpage
\begin{figure}[h!]
<<fig_3, echo=FALSE, fig=TRUE, sanitize=TRUE, out.width='5in', warning=FALSE, cache=TRUE>>=
library(ggplot2)
library(reshape2)
library(tikzDevice)
