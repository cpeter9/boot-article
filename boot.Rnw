...
A major assumption underlying the two-sample t-test is that the means of the two samples are normally distributed.  However, in the experiment described above, only one such mean for each group is produced.  Researchers could analyze the underlying structure of each sample group's token earning.  In this case, the underlying data is not normally distributed.  Are then these data suitable for a two-sample t-test?\\

Suppose that we collect data on token earning prior to our experiment.  In this case the data from two randomly selected groups looks as is shown in Figure~\ref{prob_1_raw_data} in Appendix A.  The data are clearly right skewed and look F, Chi-square, or exponentially distributed.  However, this does not disqualify these data from being analyzed with a two-sample t-test.  While the t-test is usually robust to departures from normality in large samples, it is costly for the company to run large experiments.  Despite the raw samples being non-normal, the two-sample t-test is valid if the mean difference between the two samples is normally distributed.
